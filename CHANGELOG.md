...
### Changed
- Display name changed from "GLPI Experience Kit" to "Experience Kit for GLPI" (`plugin_version_experiencekit()`
  in `setup.php`, README, ADMIN_GUIDE, and all "Generated by..." content strings). The plugin's internal
  key/namespace/table prefix (`experiencekit`) is unaffected - only the human-facing name changed.
